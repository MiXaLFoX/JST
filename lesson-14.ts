...
function reverseArr(arr: number[] | string[] | [boolean, number, string]) {
  return arr.reverse();
}

console.log(reverseArr([1, 2, 3, 4, 5]));
console.log(reverseArr(["a", "b", "c"]));
console.log(reverseArr([true, 1, "2"]));

// task 2

function maxNumInArr(arr: number[]) {
...
function fibsInArr(n: number, m: number): number[] {
  let a = 1;
  let b = 1;
  let arr = [1, 1];
  for (let i = 3; i < n + m; i++) {
...
function qualNums(firstNum: number, secondNum: number) {
...
function sortArrUp(arr: number[]) {
  return arr.sort((a, b) => a - b);
}

function sortArrDown(arr: number[]) {
